...
- **Vercel function crashes with `ERR_MODULE_NOT_FOUND`:** `@vercel/node` only bundles the entrypoint file — it does **not** follow relative imports. If the handler imports from `../src/`, those files won't exist at runtime. Fix: `api/build-vercel.mjs` uses esbuild to bundle everything into a single `api/index.mjs` at build time. Never point `vercel.json` `functions` at raw `.ts` source files that have sibling directory imports.
- **`vercel dev` re-links to the wrong project:** The `vercel dev` CLI uses `rootDirectory` from `.vercel/project.json`. Background terminals reset to workspace root — always run `vercel dev` from within `api/` in a foreground terminal. If it re-links, delete the stale `.vercel/` at repo root.
